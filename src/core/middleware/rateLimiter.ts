...
if (REDIS_URL) {
  ...
  logger.warn("rate_limiter.no_redis", {
    msg: "REDIS_URL not set — using in-memory token-bucket limiter (dev only)",
  });
}
...
export async function rateLimiter(
  ...
  try {
    ...
    res.setHeader(
      "X-RateLimit-Reset",
      String(
        now + Math.ceil((capacity - tokensLeft) / Math.max(0.1, refillPerSec)),
      ),
    );
    ...
    // Check usage quotas
    try {
      // Use baseUrl + path to get the full route path (e.g., /v1/chains instead of just /)
      // Normalize by removing trailing slash to match database entries
      ...
      if (quotaRes.remainingPaid !== undefined) {
        res.setHeader("X-Quota-Paid-Remaining", String(quotaRes.remainingPaid));
      }
      if (quotaRes.remainingFree !== undefined) {
        res.setHeader("X-Quota-Free-Remaining", String(quotaRes.remainingFree));
      }
    } catch (e: any) {
